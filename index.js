...
function findLastEvenNumber(arr) {
  ...
  let lastevenNumber = arr.findLast(function (num) {
    return num % 2 === 0;
  });
...
function getArrayKeys(arr) {
  ...
  var arrayKeys = Array.from(arr.keys());
...
function flattenSquareAndFilter(arr, threshold) {
  ...
  const squaredArray = flatedArray.map((num) => num ** 2);
  // За допомогою методу filter() фільтруємо елементи, які більше за порогове значення.
  const filteredArray = squaredArray.filter((num) => num > threshold);
...
function getLastElementsSumAndPrint(arr, n) {
  ...
  const sum = lastElements.reduceRight((acc, curr) => acc + curr);
...
function findLongestWord(arr) {
  ...
  const longestword = arr.reduce((prevWord, currentWord) => {
...
function capitalizeWords(arr) {
  ...
  const capitalizeWords = arr.map(word => {
...
function findLastNegativeNumberIndex(arr) {
  ...
  var lastIndex = arr.findLastIndex(function (num) {
    return num < 0;
  });
